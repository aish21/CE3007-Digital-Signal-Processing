...
def q2_3():
    ...
    W_angle = np.angle(W)
    # the length is 1, we are only interested in the angle of each phasor
...
def q2_2_d():
    ...
    ax[1].grid()
    plt.show()
    IDTFS_result_1 = myIDTFS(DTFS_result_1)
    IDTFS_result_2 = myIDTFS(DTFS_result_2)
    print(np.allclose(ipx_1, IDTFS_result_1))
    print(np.allclose(ipx_2, IDTFS_result_2))
    print(IDTFS_result_1)
    print(IDTFS_result_2)
...
def q2_2_b():
    ...
    ax[1].stem(DTFS_phase)
    ax[1].grid()
    for i in range(len(DTFS_magnitude)):
        ax[0].text(i, DTFS_magnitude[i], f'({i}, {DTFS_magnitude[i]:.2f})', ha='center', va='bottom', fontsize=8)
    for i in range(len(DTFS_phase)):
        ax[1].text(i, DTFS_phase[i], f'({i}, {DTFS_phase[i]:.2f})', ha='center', va='bottom', fontsize=8)
    plt.show()
    ...
    ax[1].stem(DFT_phase)
    ax[1].grid()
    for i in range(len(DFT_magnitude)):
        ax[0].text(i, DFT_magnitude[i], f'({i}, {DFT_magnitude[i]:.2f})', ha='center', va='bottom', fontsize=8)
    for i in range(len(DFT_phase)):
        ax[1].text(i, DFT_phase[i], f'({i}, {DFT_phase[i]:.2f})', ha='center', va='bottom', fontsize=8)
    plt.show()
